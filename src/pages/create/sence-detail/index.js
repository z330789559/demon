    ...
    return <View className='form'>
      <View className='form-frame'>
        <View className='form-frame-content'>
          <Header  header={header} mode={this.state.mode} changeHeader={this.changeHeader.bind(this)} isPreview={isPreview}/>
          <View>
            {sortBody.map((item,index)=><ContentHocElement  onDispatchPropsChange={onDispatchPropsChange} element={item} changeCurrentElement={this.changeCurrentElement} isPreview={isPreview} hideWrap={hideWrap} itemStyles={itemStyles} defineState={defineState} /> )}
          </View>
        </View>
        {this.state.mode  ===PAGESTATUS.development?<AtButton className={'form-add-btn'} onClick={this.onAddIssue}>添加题目</AtButton>:null}
        {this.state.mode  ===PAGESTATUS.development?<FooterTabBars className={'form-weui-tabbar__item'}
